...
if energy_eval == "SV":
    best_results = [best_results["SV"]]
else:
    best_results = list(best_results.values())

for energy_m in best_results:
    for input_file in energy_m.keys():
        instance_results = energy_m[input_file]
        for depth in instance_results.keys():
            if not (instance_type in input_file):
                continue
            if "regular" in instance_type:
                instances_subdir = instances_path / "random_regular"
            elif "swap" in instance_type:
                instances_subdir = instances_path / "line_to_full"
            elif "erdosrenyi" in instance_type:
